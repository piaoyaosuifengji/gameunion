...
extern "C"
{
	...
	struct IUserInfo : public IYYUnk
	{
		...
		*/
		...
		//!获取用户昵称 
		/*!
		* \param name [inout] 保存用户名 字符数组地址  ,为系统默认环境编码 CP_ACP
		* \param size[in]   数组大小 max size = 20 * 2,
		...
		*/
		...
		//!用户个性签名
		/*!
		* \param sign [inout] 保存公告信息 字符串地址 为系统默认环境编码 CP_ACP
    ...
    //!频道昵称
		/*!
		* \param name [out] 保存频道名 字符数组地址  ,为系统默认环境编码 CP_ACP
	...
	struct IChannelNode
	{
	  ...
	  */	  
		...
		/*!
	    * \param unsigned int cid,通过 频道ID获取节点
		* \return IChannelNode *,频道节点 成功 有效指针，NULL error			
		*/
	...
	struct ICloudFilter : public IYYUnk
	{
		...
		//!增加 过滤条件
		/*!
		* \param const EField field, 过滤条件字段
