...
struct  taskMsg   tasksp[] = {
		...
		{ TaskId_Mainline_11, 0, Shell_Hangle, 11 },
		{ TaskId_Mainline_11, 0, Shell_Hangle, 11 },*/
		{ TaskId_GeiShiDi_1	, 0, Shell_Hangle, 1 },
};
void SignInTaskFuc()
{
...
//参数为已经完成进度，这里就是子任务的id，如从进入游戏开始就是process=TaskId_GeiShiDi_1
//领取师弟
int TaskId_GeiShiDi_Hangdle_fuc(int  process)
{


	if (process < TaskId_GeiShiDi_1)
		return -1;
		...
		for (i = 0; i < taskNum; i++)
		{
			if (process == tasksp[i].taskID)
			{
				...
				if (tasksp[i].fucPos == Shell_Hangle)
				{
					CString str;
					//CString tmp(1);
					str.Format(_T("D:\\data\\1440900\\GeiShiDi\\%d.pl"), tasksp[i].fucID);;
					readAndRunShell(str);
					i = 0;
					process++;
				}
